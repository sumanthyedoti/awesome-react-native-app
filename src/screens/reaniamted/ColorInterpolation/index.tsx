...
const ICON_SIZE = CIRCLE_SIZE * 2.9
...
const ColorInterpolation: React.FC = () => {
  ...
  const rIconViewStyles = useAnimatedStyle(() => {
    const degrees = interpolate(isDark01.value, [1, 0], [90, 0])
    return {
      transform: [
        {
          rotate: degrees + 'deg',
  ...
  return (
    <Animated.View style={[styles.container, rContainerStyles]}>
      <Animated.View style={[rIconViewStyles]}>
        <Svg width={ICON_SIZE} height={ICON_SIZE} viewBox="0 0 512 512">
          <AnimatedPath
            animatedProps={rIconStyles}
